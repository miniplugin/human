...
- 작업준비: 노드MCU보드,미니USB케이블,저항220옴,점퍼케이블3개,LED1개
- 개발환경: 아두이노IDE, UART드라이버 설치
- 수행순서(아래)

```
1. 장비 목록 갭쳐
2. UART드라이버설치 후 포트 확인화면
3. 아두이노IDE에서 노드MCU(ESP8266보드셋팅) 셋팅 확인
4. C 프로그램 작성, 실행
5. 드라이버 구현 핀 연결, 사용함수 정리
6. 웹서버구현 드라이버 소스, 실행 과정
7. 실행 결과 갭쳐
8. 개선 사항 정리
```
- UART드라이버 윈10용 다운로드 [download this](git_img/CP210x_wind10.zip)
- UART드라이버 윈7용 다운로드 [download this](git_img/CP210x_win7.zip)
- 작업소스 2개 다운로드(아래)
