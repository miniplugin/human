/*
Serial 통신을 이용해서 라인(\n으로 구분)단위로 데이터를 입력받는다.(Canonical mode)
입력받은 데이터가 "z"로 시작하는 경우 프로그램을 종료한다.
...
main() {
    ...
    // controlling tty because we don't want to get killed if linenoise sent
    ...
    // terminal settings done, now handle input In this example,
    //  inputting a 'z' at the beginning of a line will exit the program.
    while(STOP == FALSE) {
        /* loop until we have a terminating condition */
        /* read blocks program execution until a line terminating
        character is input, even if more than 255 chars are input. If the
        number of characters read is smaller than the number of chars
        available, subsequent reads will return the remaining chars. res
        will be set to the actual number of characters actually read */
        res = read(fd, buf, 255);
        if(res > 0) {
