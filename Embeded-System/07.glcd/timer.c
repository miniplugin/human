...
int frameBufferInit(void){
    ...
    pfbdata = (unsigned int*)mmap(0, fvs.xres*fvs.yres*32/8,
...
int main(int argc, char *argv[]) {
	...
	if(sigaction(SIG, &sa, NULL) == -1) // timer가 expire되면 SIG신호를 내보낸다.
		errExit("sigaction"); 
	...
	// 기본상태: SIG가 mask되어있어서 signal이 보내져도 작동 안함.
	// 그 동안에 시간/타이머 등 초기화 진행
